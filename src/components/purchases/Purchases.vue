  ...
  <div>
    ...
    <div class="container">
      <div v-if="purchases">
        ...
        >
          <template v-slot:top>
            <v-toolbar flat color="blue darken-3" class="mb-2">
              <v-toolbar-title class="white--text"
                >Purchases list</v-toolbar-title
              >
              <v-spacer></v-spacer>
              ...
              <v-select
                ...
                label="Sort by"
              ></v-select>
              <v-btn-toggle class="mr-2 ml-2" v-model="sortDesc" mandatory>
                <v-btn plain large depressed color="blue" :value="0">
                  <v-icon>mdi-arrow-up</v-icon>
                </v-btn>
                <v-btn plain large depressed color="blue" :value="1">
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Created", sortable: false, value: "created_at", align: "end" }
      ],
      specIdRules: [
        v => !!v || "Specification Id is required",
        v =>
          v.length >= 4 || "Specification Id must be least than 4 characters",
        v =>
          /^[A-Za-z0-9]+$/.test(v) ||
  ...
  computed: mapState(["purchase", "purchases", "purchasesMeta"]),
  methods: {
    ...mapActions(["fetchPurchases", "createPurchase"]),
    close() {
      this.dialog = false;
    },
    ...
    async addPurchase(data) {
      ...
      this.createPurchase(data).then(() => {
        this.alertMessage = `Purchase ${
          this.purchase.title
        } was successfully created at ${new Date(
